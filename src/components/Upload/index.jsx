...
/*如果为空则创建目录*/
export function mkDir(defaultPath) {
    return __awaiter(this, void 0, void 0, function* () {
        const isExists = yield RNFS.exists(defaultPath);
        if (!isExists) {
            const options = {
                NSURLIsExcludedFromBackupKey: true, // iOS only
            };
            return yield RNFS.mkdir(defaultPath, options)
                .then((res) => {
...
export const headersOptions = {
    ...
    'Blade-Auth': 'bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9..uJLk2xQRkTaREPFMEHGGk1gNTzl2M2NfiN54APxT7yWQXdmtXf1z4YCorYuZLsUfceRUdAMuDQ2XRT6hfx6OJw'
};
export const uploadUrl = 'http://li.yunlink.win:16995/api/blade-resource/oss/endpoint/put-file-attach';
/**
 * 上传
 */
class Upload extends PureComponent {
    constructor() {
        ...
        this.handleUpload = (picker) => __awaiter(this, void 0, void 0, function* () {
        ...
        this.onPress = (flag) => {
            switch (flag) {
                case 'file':
                    ...
                    break;
                case 'picture-card':
                    MyDocumentPicker.pickerSingleFile(this.handleUpload, this.handleError, [DocumentPicker.types.images]);
                    break;
            }
        };
        this.handleOperation = (ref, item, index) => {
            // this.handleDownload(item)
            ref.measureInWindow((x, y, width, height) => {
                let items = [];
                const { remove, download } = this.props;
                if (remove)
                    items.push({ title: '删除', onPress: () => this.handleDeleteFile(index) });
                if (download)
                    items.push({ title: '下载', onPress: () => this.handleDownload(item) });
        ...
        //下载
        this.handleDownload = (item) => __awaiter(this, void 0, void 0, function* () {
    ...
    get file() {
        ...
        return (<View style={uploadContainer}>
                ...
                return <Touchable key={item.attachId || index} style={styles.touchView} onPress={() => this.handleOperation(this.['uploadRef' + (item.attachId || index)], item, index)} ref={(ref) => (this.['uploadRef' + (item.attachId || index)] = ref)}>
    ...
    get pictureCard() {
        ...
        return (<View style={[styles.pictureCardView, pictureCardView]}>
                <CircularProgress visible={visible} fill={fill} content={content}/>
                {fileList.length > 0 && fileList.map((item, index) => {
                return <Touchable key={item.attachId || index} style={styles.pickerView} onPress={() => this.handleOperation(this.['uploadRef' + (item.attachId || index)], item, index)} ref={(ref) => (this.['uploadRef' + (item.attachId || index)] = ref)}>
